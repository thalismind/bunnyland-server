...
class ColonyGenerationEnricher:
    ...
    def enrich(self, request: GenerationRequest) -> GenerationDelta:
        ...
        if ctx.is_room:
            ...
            if generation_wants(ctx, "bunnyland.colonysim.room-role") or generation_mentions(
                ctx, "barracks", "clinic", "dining room"
            ):
                add(RoomRoleComponent(role=ctx.biome or "room"))
            if generation_wants(ctx, "bunnyland.colonysim.room-stat") or generation_mentions(
                ctx, "beautiful", "clean", "comfortable"
            ):
                add(RoomStatComponent(beauty=1.0, cleanliness=1.0, comfort=1.0, wealth=25.0))
            if generation_wants(ctx, "bunnyland.colonysim.room-quality") or generation_mentions(
            ...
            if generation_wants(ctx, "bunnyland.colonysim.colony-wealth") or generation_mentions(
            ...
            if generation_wants(ctx, "bunnyland.colonysim.pawn-profile") or generation_mentions(
            ...
            if generation_wants(
                ctx, "bunnyland.colonysim.prisoner", "bunnyland.colonysim.captive"
            ) or generation_mentions(ctx, "prisoner", "captive"):
                add(PrisonerComponent(policy="hold"))
            if generation_wants(ctx, "bunnyland.colonysim.work-priority") or generation_mentions(
                ctx, "work priority"
            ):
                add(WorkPriorityComponent(priorities={generation_resource_type(ctx): 1}))
            if generation_wants(ctx, "bunnyland.colonysim.work-capability") or generation_mentions(
            ...
            if generation_wants(ctx, "bunnyland.colonysim.infection") or generation_mentions(
                ctx, "infection", "infected"
            ):
                add(InfectionComponent(severity=0.1, last_updated_epoch=ctx.world_epoch))
            if generation_wants(ctx, "bunnyland.colonysim.mental-state") or generation_mentions(
            ...
            if generation_wants(ctx, "bunnyland.colonysim.resource-node") or generation_mentions(
                ctx, "vein", "deposit", "patch"
            ):
                add(ResourceNodeComponent(resource_type=resource_type, current=5, maximum=5))
            if generation_wants(ctx, "bunnyland.colonysim.resource-stack") or generation_mentions(
                ctx, "stack", "pile of"
            ):
                add(ResourceStackComponent(resource_type=resource_type, quantity=5))
            if generation_wants(ctx, "bunnyland.colonysim.stockpile"):
                add(StockpileComponent(capacity=20))
            if generation_wants(ctx, "bunnyland.colonysim.storage-filter") or generation_mentions(
                ctx, "storage filter"
            ):
                add(StorageFilterComponent(allowed_types=(resource_type,)))
            if generation_wants(ctx, "bunnyland.colonysim.haulable") or generation_mentions(
                ctx, "haulable"
            ):
                add(HaulableComponent(priority=1))
            if generation_wants(ctx, "bunnyland.colonysim.forbidden") or generation_mentions(
                ctx, "forbidden"
            ):
                add(ForbiddenComponent())
            if generation_wants(ctx, "bunnyland.colonysim.workstation") or generation_mentions(
                ctx, "workbench", "forge", "bench"
            ):
                add(WorkstationComponent(station_type=resource_type))
            if generation_wants(ctx, "bunnyland.colonysim.recipe") or generation_mentions(
                ctx, "recipe"
            ):
                add(
                    RecipeComponent(
            ...
            if generation_wants(ctx, "bunnyland.colonysim.job"):
                add(JobComponent(job_type=resource_type, priority=1))
            if generation_wants(ctx, "bunnyland.colonysim.job-bill") or generation_mentions(
                ctx, "bill", "work order"
            ):
                add(JobBillComponent(recipe_id=resource_type, work_required=5.0))
            if generation_wants(ctx, "bunnyland.colonysim.research") or generation_mentions(
                ctx, "research", "technology"
            ):
                add(ResearchProjectComponent(project_id=resource_type, work_required=10.0))
            if generation_wants(ctx, "bunnyland.colonysim.incident") or generation_mentions(
                ctx, "incident", "raid", "blight"
            ):
                add(ColonyIncidentComponent(incident_type=resource_type))
            if generation_wants(ctx, "bunnyland.colonysim.trade-offer") or generation_mentions(
                ctx, "trade", "trader"
            ):
                add(TradeOfferComponent(faction_id="generated-faction", gives={resource_type: 1}))
            if generation_wants(ctx, "bunnyland.colonysim.surgery") or generation_mentions(
                ctx, "surgery", "operation"
            ):
                add(SurgeryBillComponent(part="torso", operation=resource_type))
            if generation_wants(ctx, "bunnyland.colonysim.body-part") or generation_mentions(
                ctx, "body part", "limb"
            ):
                add(BodyPartHealthComponent(part=resource_type))
            if generation_wants(ctx, "bunnyland.colonysim.tech-unlock") or generation_mentions(
                ctx, "unlocked tech"
            ):
                add(TechUnlockComponent(tech_id=resource_type, unlocked_at_epoch=ctx.world_epoch))
            if generation_wants(ctx, "bunnyland.colonysim.faction-relation") or generation_mentions(
                ctx, "faction relation"
            ):
                add(
                    FactionRelationComponent(faction_id=generation_trade_faction(ctx), goodwill=1.0)
                )
            if generation_wants(ctx, "bunnyland.colonysim.caravan") or generation_mentions(
                ctx, "caravan"
            ):
                add(
                    CaravanComponent(
            ...
            if generation_wants(ctx, "bunnyland.colonysim.medicine") or generation_mentions(
                ctx, "medicine", "medkit"
            ):
                add(MedicineComponent(quality=1.0, uses=1))
            if generation_wants(ctx, "bunnyland.colonysim.medical-bed") or generation_mentions(
                ctx, "clinic bed", "medical bed"
            ):
                add(MedicalBedComponent())
            if generation_wants(ctx, "bunnyland.colonysim.prosthetic") or generation_mentions(
                ctx, "prosthetic"
            ):
