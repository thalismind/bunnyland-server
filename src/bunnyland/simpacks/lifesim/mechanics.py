...
class AdoptChildHandler:
    ...
    def execute(self, ctx: HandlerContext, command: SubmittedCommand) -> HandlerResult:
        ...
        if (
            not child.has_component(LifeStageComponent)
            or child.get_component(LifeStageComponent).stage != "child"
        ):
...
class AgingConsequence:
    ...
    def process(self, world: World, epoch: int):
        ...
        for entity in query.execute_entities():
            ...
            if age >= policy.natural_death_age_seconds and not entity.has_component(
                DownedComponent
            ):
    ...
    for edge, decedent_id in character.get_relationships(InheritedFrom):
        ...
        if record_id is not None and world.has_entity(record_id):
            ...
            if record_entity.has_component(InheritanceRecordComponent):
                inheritances.append(record_entity.get_component(InheritanceRecordComponent).summary)
                continue
