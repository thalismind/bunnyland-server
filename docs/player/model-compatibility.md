...
the documentation-only commit `c3f2729`. Their unchanged Apple and Clover cells extend
`v2`, while their Bell cells extend `v4`; the exact commit remains recorded as provenance.
Runs used five fresh worlds per model/tutorial pair, provider-default temperature, and high
thinking where the model supported it.
