...
World history is durable ECS state for notable shared events. Physical marks and creator
signatures are durable ECS state for authored changes and made objects. None of these are
private memory or narration. Narration and prompts may present these records, but the
source of truth is ECS state.
...
circumstances. All include ids and source event ids for audit.
